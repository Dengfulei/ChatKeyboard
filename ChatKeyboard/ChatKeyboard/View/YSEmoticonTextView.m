...
@implementation YSEmoticonTextView
...
- (void)insertEmoticon:(YSEmoticonModel *)emotion
{
    if (emotion.code) {
        ...
        // 加载图片
        YSEmoticonAttachment *attch = [[YSEmoticonAttachment alloc] init];
        
        // 传递模型
        attch.emoticonModel = emotion;
...
- (NSString *)fullText
{
    ...
    [self.attributedText enumerateAttributesInRange:NSMakeRange(0, self.attributedText.length) options:0 usingBlock:^(NSDictionary *attrs, NSRange range, BOOL *stop) {
        // 如果是图片表情
        YSEmoticonAttachment *attch = attrs[@"NSAttachment"];
        if (attch) { // 图片
            [fullText appendString:attch.emoticonModel.chs];
        } else { // emoji、普通文本
            // 获得这个范围内的文字
